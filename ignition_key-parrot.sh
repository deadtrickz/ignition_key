...
#VSCodium is used instead and installed on ParrotOS
...
wget "https://www.realvnc.com/download/file/viewer.files/VNC-Viewer-6.20.529-Linux-x64.deb" -O vnc_viewer.deb
dpkg -i vnc_viewer.deb
rm vnc_viewer.deb

BLUE "Install Real VNC Connect (Server)..."
wget 'https://www.realvnc.com/download/file/vnc.files/VNC-Server-6.7.2-Linux-x64.deb' -O vnc_server.deb
...
echo -e "\ndeb https://typora.io/linux ./" | sudo tee -a /etc/apt/sources.list.d/parrot-custom.list 
sudo apt-get update

# install typora (no repo in parrot)
